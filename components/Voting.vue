<template>
  ...
  py-1 px-2 rounded-full ${upVoted ? 'bg-red-100' : ''}`" @click="toggleUpVoted">
    <p class="py-0 my-0 leading-4">
      <span class="counter text-xs leading-5 py-0 pr-1 text-zinc-900 h-full">
        {{ votes }}
      </span>
      <Icon v-show="!upVoted" name="material-symbols:favorite-outline" class="text-lg leading-5 text-red-600" />
      <Icon v-show="upVoted" name="material-symbols:favorite" class="text-lg text-red-600" />
    </p>
  </div>
</template>
